...
std::tuple<common::utils::LevelOrderedCircuit, std::vector<std::vector<std::vector<common::utils::wire_t>>>> generateCircuit(size_t n, size_t l, std::vector<Ring> &weights) {
    ...
    auto output_vector = subcirc::pi_3_reference(adj_matrices, circ, n, weights);
...
void benchmark(const bpo::variables_map& opts) {
    ...
    */
    ...
    for (size_t r = 0; r < repeat; ++r) {
        ...
        if (pid != 0) {
            assert(bytes_sent == 8*n*n*(weights.size()-1) + 4*n);
            assert(bytes_sent_pre == 0);
        } else {
            assert(bytes_sent == 0);
            assert(bytes_sent_pre - 56 == 4*n*n*(weights.size()-1)); // 56 always sent to synchronize vector sizes
        }
        assert(circ.gates_by_level.size() == weights.size());
...
int main(int argc, char* argv[]) {
    ...
    bpo::options_description cmdline(
      "Benchmark reference pi_3 based on matrix multiplication");
