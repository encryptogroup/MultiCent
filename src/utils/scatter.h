...
struct Node {
    ...
    Node() = default;
    Node(wire_t src, wire_t dest, wire_t datas, wire_t datag)
        :   src{src}, dest{dest}, datas{datas}, datag{datag} {}
};
...
class Scatter {
    ...
    public:
     explicit Scatter(size_t dagsize)
        :   dagsize{dagsize} {}
    ...
    const Circuit<R>& getScatterCircuit() {
        size_t N = dag.nodes.size();
