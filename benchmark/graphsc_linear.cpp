...
common::utils::Circuit<Ring> generateCircuit(size_t vec_size) {
    ...
    std::vector<common::utils::wire_t> tmp1(vec_size);
    tmp1[0] = zero;
    for (int i = 1; i < vec_size; ++i) {
        dag_list_data[i] = tmp1[i-1];
        tmp1[i] = circ.addGate(common::utils::GateType::kMul, dag_list_isV[i], tmp1[i-1]);
    }
    ...
    // GATHER
    std::vector<common::utils::wire_t> agg(vec_size);

    agg[0] = zero;
    for (int i = 1; i < vec_size; ++i) {
        dag_list_data[i] = agg[i-1];
        auto tmp = circ.addGate(common::utils::GateType::kAdd, agg[i-1], dag_list_data[i]);
        auto tmp_ = circ.addGate(common::utils::GateType::kMul, tmp, dag_list_isV[i]);
        agg[i] = circ.addGate(common::utils::GateType::kSub, tmp, tmp_);
    }
