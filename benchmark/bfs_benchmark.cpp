...
std::tuple<common::utils::LevelOrderedCircuit, std::vector<std::vector<common::utils::wire_t>>,
            ...
            std::vector<common::utils::wire_t>, std::vector<common::utils::wire_t>> generateCircuit(size_t n, size_t total_size, size_t nmbr_bits, size_t depth) {
    ...
    auto output_vector = subcirc::bfs(source_bits, destination_bits, vertex_flags, payload, circ, next_free_shuffle_id, n, nmbr_bits, depth);
...
void benchmark(const bpo::variables_map& opts) {
    ...
    */
    ...
    for (size_t r = 0; r < repeat; ++r) {
        ...
        std::cout << "OnlineEvaluator constructed" << std::endl;
        ...
        // if (pid != 0) {
        //     assert(bytes_sent == 48 * size * nmbr_bits + 68 * size + 16 * size * weights.size() + 4 * n);
        //     assert(bytes_sent_pre == 0);
        // } else {
        //     assert(bytes_sent == 0);
        //     if (weights.size() > 0) 
        //         assert(bytes_sent_pre - 56 == 64 * size * nmbr_bits + 108 * size + 32 * size * weights.size()); // 56 always sent to synchronize vector sizes
        //     else {
        //         // Lazy evaluation: double-shuffles are not set up if we use no iteration, each is 8t setup, so we expect 24t setup less
        //         assert(bytes_sent_pre - 56 == 64 * size * nmbr_bits + 108 * size + 32 * size * weights.size() - 24 * size);
        //     }
        // }
